...
########### beta diversity plot
betadiv_plot <- function(phydat, nrow, scale) {
  immc.ord <- ordinate(phydat, "NMDS", "bray")
  p <- plot_ordination(phydat, immc.ord,  color="group", 
                       title="Taxonomy") +  
    geom_point( size=3)+
    theme_bw() 
...
########### alpha diversity plot (richness)

plotrichness <- function(phylodat, measure ) {
  p1<- plot_richness(phylodat, measures= measure )
  df <- p1$data 
  
  df0 <- df %>% group_by(group, type0,type3) %>% 
    summarise(div_mean = mean(value), Error= sd(value)) 

  ggplot(df0,  aes(x = group, y = div_mean))+
    geom_col(aes(fill = group), width = 0.7)+
    geom_errorbar(aes(ymin=div_mean-Error, ymax=div_mean+Error),
                  linewidth=.5,  
                  width=.2,
                  position=position_dodge(0.9))+
    xlab("")+ ylab(paste0("Alpha diversity (",measure,")"))+
    theme_minimal() +
    scale_fill_manual("Method", values =c("darkcyan", "darkgoldenrod2", "#FC4E07"))+
    theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))+
    facet_grid(. ~ type0 + type3)
